...
INSERT INTO `login_credentials` (`email`, `password`, `date_added`) VALUES
...
CREATE TABLE `login_user_ip` (
  ...
  KEY `email_2` (`email`),
  CONSTRAINT `USERIP_EMAIL_FK` FOREIGN KEY (`email`) REFERENCES `login_credentials` (`email`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `admin` (
  ...
  CONSTRAINT `ADMIN_EMAIL_FK` FOREIGN KEY (`email`) REFERENCES `login_credentials` (`email`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=latin1;

INSERT INTO `admin` (`admin_id`, `first_name`, `last_name`, `email`) VALUES
(1, 'John', 'Admin', 'admin@swadesh.com');
...
CREATE TABLE `staff` (
  ...
  CONSTRAINT `STAFF_EMAIL_FK` FOREIGN KEY (`email`) REFERENCES `login_credentials` (`email`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=latin1;
...
CREATE TABLE `customers` (
  ...
  CONSTRAINT `CUST_EMAIL_FK` FOREIGN KEY (`email`) REFERENCES `login_credentials` (`email`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=latin1;
